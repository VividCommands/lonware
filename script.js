...
document.addEventListener('DOMContentLoaded', () => {
        ...
        const navbar = document.querySelector('.navbar');
                              ...
                              // Set initial navbar top based on announcement bar height
                              const updateNavbarTop = () => {
                                          if (!navbar) return;
                                          if (announcementBar) {
                                                          const barHeight = announcementBar.offsetHeight;
                                                          if (window.scrollY < barHeight) {
                                                                              // Announcement bar is still (partially) visible - offset navbar by remaining height
                                                              navbar.style.top = (barHeight - window.scrollY) + 'px';
                                                          } else {
                                                                              // Announcement bar has scrolled out of view - navbar goes to top
                              ...
                              // Set the correct top on page load
                              updateNavbarTop();
        ...
        if (purchaseFeedList) {
            ...
            const names = [
                            // Extended name list for the purchase feed.
            ...
            setInterval(addPurchaseEntry, 6000);
                              ...
                              // hovers over the card and reset when the pointer leaves.
