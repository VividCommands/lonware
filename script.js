...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateNavbar = () => { if (!navbar) return; navbar.classList.toggle('scrolled', window.scrollY > getAnnH()); };
    ...
    (function initParticles() {
          ...
          function animate() {
                  ...
                  for (let i=0;i<particles.length;i++) {
                            const p=particles[i]; p.x+=p.vx; p.y+=p.vy; p.pulsePhase+=p.pulseSpeed; p.opacity=p.baseOpacity+Math.sin(p.pulsePhase)*0.04;
                            ...
                            const spd=Math.sqrt(p.vx*p.vx+p.vy*p.vy); if(spd>0.5){p.vx=p.vx/spd*0.5;p.vy=p.vy/spd*0.5;}
